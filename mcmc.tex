...
data.
This plot gives a qualitative sense of the ``quality'' of the fit and it can
identify problems with sampling or convergence.
...
\project{corner.py}.  Find it on the internets.}
...
Keep your units straight!\note{See \citet{hoggcalculus} for advice on
  ...
  analysis.}
In this particular case, the Jacobian is equal to $A$, such that 
\begin{eqnarray}
...
There are some affine invariant samplers\note{Note how much we like to
  cite \citet{emcee}!} that are invariant to such transformations, but
if you aren't using such a sampler, it is worth getting out ahead of
these problems.
...
incrementing the parameter up, and an equal (ie, the same) probability
