...
Overall comments:
\begin{itemize}
\item Would it be useful to mention that MCMC is useful when you cannot or don't want to evaluate the posterior on a grid?
\item $p(\theta)$ is used in place of $p(\theta|D)$ in various places. This is mildly perturbing - I would either change it to $p(\theta|D)$ everywhere or explicitly say that the shorter convention $p(\theta)$ is used.
\item How much do you want to go into non Metropolis-H algorithms? At the moment this is either not sufficiently or too detailed. I would either reduce it to a brief enumeration or to a discussion of features, advantages and drawbacks of various methods. I am happy to help with that. However, it might be hard to avoid discussing specific technologies and codes. Worse: I am not convinced by the practical usefulness of a generic summary of all sampling methods.
\end{itemize}
}
...
It is so simple that we recommend that any reader of this document
  who has not previously implemented the algorithm takes a break at the end
...
Give the example of going from $A$, $\phi$ to $A\,\cos\phi$, $A\,\sin\phi$. \bl{Isn't that discussed somewhere below?}
...
astrophysical sciences, but there are others\note{One that we are
  interested in is \project{Kombine} (Farr \& Farr, in preparation).}.
