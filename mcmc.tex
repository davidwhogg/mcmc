...
% to-do
% -----
% - Kill all issues in this list, below
...
% - Mention & cite JAGS and STAN in methods section
% - Do we explain how to initialize emcee; do we need to?
...
You can break this property, if you like, and then adjust the
  ...
  acceptance fraction.  Read the paper \cite{emcee} for more details.}
...
problem.\note{Famously, there is ``no free lunch''
  ...
  optimization problem without exhastive search of all possibilities.
...
DFM: Here or in a note, can you give an empirical estimator of the
autocorrelation time?  The naive estimator is good to give, and we
could also give a good estimator too.
...
and we will (below; \sectionname~\ref{sec:methods}) tune the samplers we have to do the best on the
...
HOGG: Introduce and explain the Gelman--Rubin
diagnostic\note{\citet{gelmanrubin}.}, which is also a good idea, and
which tests something slightly different than the autocorrelation
time.
...
\begin{problem}
Look at the source code of \project{emcee} \citep{emcee}, and see if you can
...
Most \MCMC\ methods
  ...
  nearly-fixed proposal distribution and requires no tuning \citep{emcee}.
  There are many problems for which this is \emph{not} the best choice
  of sampler, but since it obviates tuning, it can be a good first choice.})
...
\project{emcee} sampler (see \citealt{emcee}).
...
One thing we can keep in mind to guide us in this is what we said above (\sectionname{sec:sampling}),
...
produces the gradient of any function you can write.\note{%
  ...
  function, using simple differentiation rules andthe magic of the
