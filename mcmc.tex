...
% Copyright 2013, 2014 David W. Hogg (NYU) and Dan Foreman-Mackey (NYU).
...
Finally---and perhaps most importantly---another magical property of a sampling
...
The simplest algorithm for MCMC is the Metropolis--Hastings algorithm.\note{%
...
...How to avoid violating detailed balance (without deeply understanding it)?
...
Now you have a scalar function $q(\theta)$ which takes the parameter vector
  and returns a scalar value%
  \note{For the median-of-sampling value, $q(\theta)$ needs to return a scalar,
  but technically, for the mean-of-sampling, $q(\theta)$ can be a vector or something high-dimensional.}
...
\begin{eqnarray}
\mean{q} &\leftarrow& \frac{1}{K}\,\sum_{k=1}^K q(\theta_k)
\quad ,
\end{eqnarray}
  and the median-of-sampling value is just the $[K/2]$th value of $q(\theta_k)$
  when the $q(\theta_k)$ have been ordered (sorted).
...
Again, these limits can be estimated%
  ...
  by ordering the $q(\theta_k)$ values,
  and finding the $\theta$ values such that some fraction (0.68 or 0.95)y
  of the samples lie between them.
Again, to form these limits, $q(\theta)$ must be a scalar function (so sorting is well defined).
