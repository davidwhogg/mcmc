...
inference---that the function $f(\theta)$ is the product of two
...
That the function $f(\theta)$ can be thought of as a product of a
...
\section{Gibbs, ensemble, Hamiltonian, and so on}

[SHOULD THIS SECTION MOVE TO AFTER THE DEFINITION OF AUTOCORRELATION TIME AND DISCUSSION OF CONVERGENCE?]
...
The only significant technical details for importance sampling are the
following:
...
\paragraph{Nested:}
Similar to tempering are a class of methods called ``nested''
samplings, which also smoothly change the target distribution.
...
Finally, there are some problems that \emph{cannot be done} without
additional proposal technology...
...
when taken together as a ``best-fit'' parameter vector $\theta^\ast$,
