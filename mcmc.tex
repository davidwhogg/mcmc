...
The second input is a proposal function $p(\pars'\given\pars)$,
  such that the algorithm can draw a new position $\pars'$ in the parameter space
  given an ``old'' position $\pars$.
This second function must meet some symmetry requirements we discuss further below.
It permits us to random-walk around the parameter space in a fair way.

The M--H MCMC algorithm is the following:
...
\begin{itemize}
\item Draw a proposal $\pars'$ from the proposal disribution $p(\pars'\given\pars_k)$.
\item Draw a random number $0<r<1$ from the uniform distribution.
\item If $p(\pars') / p(\pars_k) > r$ then $\pars_{k+1} \leftarrow \pars'$;
      otherwise $\pars_{k+1} \leftarrow \pars_k$.
\end{itemize}
That is, at each step, either a new proposed position in the parameter
  space gets accepted or else the prior position in the parameter space
  gets repeated.
The algorithm can be iterated a large number $K$ of times to produce $K$ samples.

...The algorithm is a biased random walk; nearby samples are covariant...etc.
...
...Cycling through parameters??  Mention here?
...
Again, these limits can be estimated%
  ...
  and finding the $\pars$ values such that some fraction (0.68 or 0.95)y
