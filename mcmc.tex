...
as it directly connects to the variances of sampling estimates of quantities of interest.
...
lunch'' theorem of discrete optimization \citep{nofreelunch}: In most problems there is no
...
%The autocorrelation function $C_g$ is defined by
%\begin{eqnarray}
%C_g(\Delta) &\equiv& \frac{E[(g(\pars_k)-\mu_g)\,(g(\pars_{k+\Delta})-\mu_g)]}
%                          {E[(g(\pars)-\mu_g)^2]}
%\label{eq:autocorrelationfn}\\
%\mu_g &=& E[g(\pars)]
%\quad ;
%\end{eqnarray}
%where $g(\pars)$ is a scalar valued function of the parameters, and $\Delta$
%is a lag (difference in index number) between points in the chain.
...
%C_g(0) &=& 1
%\\
%\lim_{\Delta\rightarrow\infty} C_g(\Delta) &=& 0
%\quad ,
%\end{eqnarray}
%and by definition, the autocorrelation ``time'' $\tau$ is
%\begin{eqnarray}
%\tau_g = 1 + 2\,\sum_{\Delta = 1}^{\infty} C_g(\Delta)
...
%\begin{eqnarray}
%  \Var[E_{p(\pars)}[g(\pars)]] &=& \frac{\tau_g}{K}\,\Var_{p(\pars)}[g(\pars)]
%  \\
%  &\approx& \frac{\tau_g}{K^2}\,\sum_{k=1}^K \left(g(\pars_k) -
...
%Instead, there is an autocorrelation time $\tau_g$ for every function
%$g(\pars)$ and these will, in general, be different.
%Therefore, the autocorrelation time used in \equationname~(\ref{eq:tauvar}) is
%the time computed on the chain of $g(\pars)$ values instead of the
...
%\emph{estimator} for the time. Estimating the auto-correlation time is
%notoriously difficult: It is a two-point statistic, and two-point statistics
%are much harder to estimate than one-point statistics.\note{See: All of cosmology!}
%If you are not a hard-core user of \MCMC, and if all you want is heuristic indicators
%of convergence, then what you should take from this section is that the auto-correlation
%time is involved in variance estimates through \equationname~(\ref{eq:tauvar}) but that
%it is hard to estimate.

%If you want to think about autocorrelation estimation:\note{One of us (DFM) has written
%  much more about autocorrelation estimation in the \project{emcee} documentation
%  at \url{http://dfm.io/posts/autocorr}.}
%You might first try the na\"ive estimator
...
%trusted if the chain is many times ($\sim 100$) longer than $\tau$.\footnote{This
%  point is also emphasized at the aforementioned tutorial
%  \url{http://dfm.io/posts/autocorr}, where
%  a number like 50 is recommended.}
...
The recipe given on page 16 of A.~Sokal's notes \citep{sokal} might be helpful.
...
\begin{eqnarray}
\mean{q} &\leftarrow& \frac{1}{K}\,\sum_{k=1}^K g(\pars_k)
\quad ,
\end{eqnarray}
...
\paragraph{Gibbs:}
Neither \MH\ \MCMC\ nor most ensemble methods good at going to very
  large numbers of parameters.\footnote{See the excellent rant ``Ensemble Methods are Doomed to Fail in High Dimensions'' at \url{http://andrewgelman.com/2017/03/15/ensemble-methods-doomed-fail-high-dimensions/}.}
...
there are good implementations.\note{For example, \project{STAN} and \project{JAGS}; find them on the internets. DFM INSERT URLs AND REFERENCES.}
