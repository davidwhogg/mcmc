...
% Copyright 2013, 2014, 2015 David W. Hogg (NYU) and Dan Foreman-Mackey (NYU, UW).

% to-do
% -----
% - Deal with all mentions of ``DFM'' or ``HOGG'' and all ``...''
...
% - when we say ``below'' also give a Section Number.
...
\footnotetext{%
    ...
    license\note{\label{note:first}
        Copyright 2013, 2014, 2015 by the authors. This work is licensed under a
...
That is, even if---in the end---%
  ...
  and given the intuition you gain by doing it (at least once) yourself.\note{HOGG Refer again to the DAR on fitting a line for an example project.}
...
\end{eqnarray}
%% DFM: IS THIS DEFINITION CORRECT OR IS THERE A FACTOR OF 2 OR SOMETHING?
...
%% HOGG:  Note here that this requires writing the f(\pars) code to capitalize on the axis-aligned moves!!
...
... HOGG more here!  For example, does this work better in large
numbers of dimensions?  If so, we should say so.
...
...Make triangle plots.  Great value!...  Show example here... Mentioned above; drop here or reiterate?
...
...The Bayes integral / evidence integral?  It is both the biggest strength and biggest weakness that vanilla MCMC methods can't give you this integral.  There are tricks, and then there are MCMC methods.  All that said, you never really want to calculate this integral!
